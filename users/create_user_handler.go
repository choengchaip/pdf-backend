...
func SetupCreateUserHandler(e *echo.Echo, ctx context.IContext) {
	e.POST("/user", func(context echo.Context) error {
		...
		result, err := service.CreateUser(params["username"].(string), params["password"].(string), params["first_name"].(string), params["last_name"].(string), params["role"].(string))
		if err != nil {
